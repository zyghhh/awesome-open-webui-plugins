...
(function() {
  ...
  function beginTickCache() {
    tickCache = Object.create(null);
  }

  function endTickCache() {
    tickCache = null;
  }
  ...
  function getSearchableText(msg, skipReasoning) {
    var cacheKey = skipReasoning ? 'strictText' : 'laxText';
    if (tickCache && Object.prototype.hasOwnProperty.call(tickCache, cacheKey)) {
      return tickCache[cacheKey];
    }
    var out = '';
    try {
      ...
      var fallback = _ivStripDetailRanges(msg.textContent || '', skipReasoning);
      if (tickCache) tickCache[cacheKey] = fallback;
      return fallback;
    }
    var result = _ivStripDetailRanges(out, skipReasoning);
    if (tickCache) tickCache[cacheKey] = result;
    return result;
  }
  ...
  function stripFinalizeArtifacts() {
    var msg = findMyMessage();
    if (!msg) return;
    try { hideMarkerRange(true); } catch(e) {}
    var nodes = [];
    try {
      var walker = parent.document.createTreeWalker(
  ...
  function resetRenderAreaForFinalPaint() {
    // Streaming paints safe prefixes into #iv-render. Browser auto-close
    // behavior can leave partial source nodes that are no longer present
    // in the final block. Rebuild final HTML from a clean root; scripts
    // have not been materialized during streaming, so this does not erase
    // library-rendered charts.
    try { renderArea.textContent = ''; } catch(e) {}
  }
  ...
  function finalize(fullText) {
    ...
    finalized = true;
    cleanupRuntimeWatchers();
    resetRenderAreaForFinalPaint();
  ...
  var TICK_MIN_INTERVAL = 120;
  var tickScheduled = false;
  var pendingForceHide = false;
  var lastTickAt = 0;
  ...
  function scheduleTick(forceHide) {
    pendingForceHide = pendingForceHide || !!forceHide;
    if (tickScheduled || finalized) return;
    var now = Date.now ? Date.now() : new Date().getTime();
    var delay = Math.max(0, TICK_MIN_INTERVAL - (now - lastTickAt));
    tickScheduled = true;
    setTimeout(function() {
      tickScheduled = false;
      lastTickAt = Date.now ? Date.now() : new Date().getTime();
      var runForceHide = pendingForceHide;
      pendingForceHide = false;
      try { tick(runForceHide); } catch(e) {}
    }, delay);
  }

  function softRenderIncomplete(raw) {
    if (finalized || !_ivLooksRenderable(raw)) return;
    var cut = findSafeCut(raw);
    var safe = raw.substring(0, cut);
    if (safe !== lastSafeRendered && safe.length > 0) {
      lastSafeRendered = safe;
  ...
  var innerMo = null;
  var innerMoTarget = null;
  var outerMo = null;
  var pollTimer = null;
  function attachInnerObserver() {
    var msg = findMyMessage();
    if (!msg) return;
    if (innerMo && innerMoTarget === msg && parent.document.contains(innerMoTarget)) return;
    if (innerMo) {
      try { innerMo.disconnect(); } catch(e) {}
      innerMo = null;
      innerMoTarget = null;
    }
    try {
      innerMo = new MutationObserver(function(records) {
        hideMarkerRangeNow();
        scheduleTick(_ivHasChildListMutation(records));
      });
      innerMo.observe(msg, {
      ...
      innerMoTarget = msg;
      if (outerMo) {
        try { outerMo.disconnect(); } catch(e) {}
        outerMo = null;
      }
    } catch(e) {}
  }
  ...
  function pollTick() {
    try { hideMarkerRangeNow(); } catch(e) {}
    try { scheduleTick(false); } catch(e) {}
  ...
  try { attachInnerObserver(); } catch(e) {}
  try {
    outerMo = new MutationObserver(function(records) {
      try { hideMarkerRangeNow(); } catch(e) {}
      try { scheduleTick(_ivHasChildListMutation(records)); } catch(e) {}
      try { attachInnerObserver(); } catch(e) {}
    });
    outerMo.observe(parent.document.body, {
      childList: true, subtree: true, characterData: true
    });
  } catch(e) {}
  pollTimer = setInterval(pollTick, 400);
})();
</script>
"""
